            ...
            <table style="margin: 20px;">
                ...
                <tr>
                    ...
                    <td><input style="" type="text" id="user_name" name="user_name"/></td>
                     <td><button class="up_pass_user">Reset mật khẩu</button></td>
                      <td><button class="activation_user">Xác thực tài khoản</button></td>
                </tr>
    ...
    function salestaff_paginate_function($item_per_page, $current_page, $total_records,$url)  {
        ...
        if($total_pages > 0 && $total_pages != 1 && $current_page <= $total_pages){ //verify total pages and current page number
            ...
            if($current_page > 1){
                ...
                $pagination .= '<li class="first"><a href="'.$url_first.'"  title="First"><i class="fa fa-angle-double-left" aria-hidden="true"></i></a></li>'; //first link
                if ($previous_link==1) {
                    $pagination .= '<li><a href="'.$url_first.'" title="Previous"><i class="fa fa-angle-left" aria-hidden="true"></i></a></li>'; //previous link
                }else{
                    $pagination .= '<li><a href="'.$url.'p='.$previous_link.'" title="Previous"><i class="fa fa-angle-left" aria-hidden="true"></i></a></li>'; //previous link
                }
            ...
            if($current_page < $total_pages){ 
                    ...
                    $pagination .= '<li><a href="'.$url.'p='.$next_link.'"  title="Next"><i class="fa fa-angle-right" aria-hidden="true"></i></a></li>'; //next link
                    $pagination .= '<li class="last"><a href="'.$url.'p='.$total_pages.'" title="Last"><i class="fa fa-angle-double-right" aria-hidden="true"></i></a></li>'; //last link
            }
...
if(isset($_GET["user_pass"]))
{
    ...
    $user = get_userdatabylogin($_GET["user_pass"]);
    if($user){
        $user_id= $user->ID;
...
if(isset($_GET["activation_user"])) {
    $user = get_userdatabylogin($_GET["activation_user"]);
    if ($user) {
        $user_id = $user->ID;
