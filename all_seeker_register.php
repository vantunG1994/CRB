...
$employer = $wpdb->get_results("SELECT DISTINCT number_phone,industry_user,date_create,status,id FROM crb_cookie_user WHERE account_type='job-seeker' ORDER BY date_create DESC  LIMIT $limit,$item_per_page");
$employer_count = $wpdb->get_var("SELECT COUNT(id) FROM crb_cookie_user WHERE account_type='job-seeker' ");
//$employer_count=count($employer);
?>
<h2>Có tất cả <?php echo $employer_count ?> ứng viên</h2>
<?php
$notetemplate=array(
'Không liên lạc được.',
...
$job_seeker = $wpdb->get_row("SELECT * FROM crb_cookie_user WHERE account_type='job-seeker' AND id='$id_user'");
//        print_r($job_offer);
?>
<div style="border:solid 1px #ccc; padding:3px; ">
    <h3 style="color: crimson;">Bạn đang hỗ trợ khách hàng <?php echo $job_seeker->number_phone; ?></h3>
    <p><b>ID:</b> <?php echo $job_seeker->id; ?></p>
    <p><b>SĐT:</b> <?php echo $job_seeker->number_phone; ?></p>
    <p><b>Ngành nghề:</b> <?php echo $job_seeker->industry_user; ?></p>
...
update_user_meta($user_id, 'account_type','job-seeker');
...
<table class="" style="margin-top: 50px;">
    <caption>Danh Sách Ứng Viên Hỗ Trợ </caption>
    <thead>
        <tr>
    ...
    <tbody>
        ...
        ?>
        <tr>
            <td><a href="<?php home_url().$_SERVER["REQUEST_URI"] ?>admin.php?page=all-seeker-register&p=<?php echo $page; ?>&id_user=<?php echo $user->id; ?>"><?php echo $user->number_phone; ?></a></td>
